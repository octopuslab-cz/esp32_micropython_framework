```
-----------------------------
components.led               1.0.0
components.button            2.0.1
components.rgb               1.0.1
components.pwm               1.0.2
components.buzzer            1.0.1
components.i2c_expander      1.1.1
components.display7          1.2.0
components.ds18d20           1.0.1
components.oled              Version information not available.
components.uart_display      Version information not available.
components.i2c_eeprom_24xxx  Version information not available.
-----------------------------
config.                      1.0.3
-----------------------------
utils.octopus_lib            1.2.0
utils.octopus_decor          0.0.3
utils.octopus_digital        0.2.1
utils.octopus_api            1.0.2
utils.pinout                 1.0.2
utils.setup                  0.8.2 # 15.07.2023
utils.sys_info               0.9.0
utils.transform              0.0.5
utils.wifi_connect           1.0.3
-----------------------------
components.servo             1.0.1
```
